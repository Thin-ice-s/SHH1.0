...
| `transfer` | **`file_stat`** | Get metadata of a local file (size, mtime, sha256). Use it before/while transferring to plan chunk offsets and to RESUME an interrupted upload/download. |
| `transfer` | **`file_upload_chunk`** | Upload ONE chunk of a file to an exact byte offset. Chunks keep request bodies small so the tunnel cannot be overloaded. Set offset=0 to start (or use truncate=true to overwrite); keep calling with the SAME file and increasing offsets to append. Returns next_offset so the transfer can resume precisely after a dropped tunnel. |
| `transfer` | **`file_download_chunk`** | Read ONE chunk of a local file from an exact byte offset and return it base64-encoded (or as utf-8 text). Small chunks are what keep tunnel transfers stable; loop over offsets until eof=true. Optionally gzip-compress the chunk to cut tunnel traffic ~3-5x for text. |
| `transfer` | **`file_checksum`** | Compute the sha256 (and md5) checksum of a local file to verify a transfer completed intact. |
| `transfer` | **`file_transfer_info`** | Explain the safe chunked-transfer strategy and the current hard size limits. Call this before moving big files so you never send an oversized packet that drops the tunnel. |
...
### `file_stat`
**功能描述**: Get metadata of a local file (size, mtime, sha256). Use it before/while transferring to plan chunk offsets and to RESUME an interrupted upload/download.  
**所属分类**: `transfer`  
...
```json
{
  ...
  "properties": {
    "path": {
      "type": "string",
      "description": "File path on the local computer."
    },
    "hash": {
      "type": "boolean",
      "default": false,
...
**调用示例 (HTTP POST /api/tools/file_stat)**:
```json
{
  "path": "C:\\projects\\demo\\app.py",
  "hash": false
...
```json
{
  ...
  "properties": {
    "path": {
      "type": "string",
      "description": "Destination file path on the local computer."
    },
    "offset": {
      "type": "integer",
      "default": 0,
...
**调用示例 (HTTP POST /api/tools/file_upload_chunk)**:
```json
{
  "path": "C:\\projects\\demo\\app.py",
  "offset": 0,
  "data_base64": "example_value",
  "data_text": "example_value",
  "compress": false,
  "truncate": false,
  "sha256_chunk": "example_value"
}
```
...
### `file_download_chunk`
**功能描述**: Read ONE chunk of a local file from an exact byte offset and return it base64-encoded (or as utf-8 text). Small chunks are what keep tunnel transfers stable; loop over offsets until eof=true. Optionally gzip-compress the chunk to cut tunnel traffic ~3-5x for text.  
**所属分类**: `transfer`  
...
```json
{
  ...
  "properties": {
    "path": {
      "type": "string",
      "description": "Source file path on the local computer."
    },
    "offset": {
      "type": "integer",
      "default": 0,
...
**调用示例 (HTTP POST /api/tools/file_download_chunk)**:
```json
{
  "path": "C:\\projects\\demo\\app.py",
  "offset": 0,
...
### `file_checksum`
**功能描述**: Compute the sha256 (and md5) checksum of a local file to verify a transfer completed intact.  
**所属分类**: `transfer`  
...
**调用示例 (HTTP POST /api/tools/file_checksum)**:
```json
{
  "path": "C:\\projects\\demo\\app.py"
}
...
### `file_transfer_info`
**功能描述**: Explain the safe chunked-transfer strategy and the current hard size limits. Call this before moving big files so you never send an oversized packet that drops the tunnel.  
**所属分类**: `transfer`  
...
**调用示例 (HTTP POST /api/tools/file_transfer_info)**:
```json
{}
```
